...
```
在上面的语法中包含三对HTML标记，分别为```md<table></table>、<tr></tr>、<td></td>，```他们是创建表格的基本标记，缺一不可，下面对他们进行具体地解释。
```md<table></table>：```用于定义一个表格。
```md<tr></tr>：```用于定义表格中的一行，必须嵌套在```md<table></table>```标记中，在```md<table></table>```中包含几对```md<tr></tr>```，就有几行表格。
```md<td></td>：```用于定义表格中的单元格，必须嵌套在```md<tr></tr>```标记中，一对```md<tr></tr>```中包含几对```md<td></td>```，就表示该行中有多少列（或多少个单元格）。

注意：
学习表格的核心是学习```md<td></td>```标记，他就像一个容器，可以容纳所有的元素
